...
final class InputToTextareaHtmlTagMutator implements Mutator
{
    ...
    {
        $open  = static fn(string $s) => preg_replace('/<\s*input(\s|\/|>)/i', '<textarea$1', $s);
        $close = static fn(string $s) => preg_replace('/<\s*\/\s*input\s*>/i', '</textarea>', $s);
    ...
    public static function getDefinition(): Definition
    {
        return new Definition(
            'Replaces <input> with <textarea> in string/encapsed HTML.',
            MutatorCategory::SEMANTIC_REDUCTION,
            null,
            <<<'DIFF'
- echo "<input name=\"msg\">";
+ echo "<textarea name=\"msg\">";
DIFF
        );
    }

...
final class StrposToStrEndsWithMutator implements Mutator
{
    ...
    public function mutate(Node $node): iterable
    {
        ...
        yield new FuncCall(new Name('str_ends_with'), [$node->args[0], $node->args[1]], $node->getAttributes());
    }

    public static function getDefinition(): Definition
    {
        return new Definition(
            'Replaces strpos($h, $n) with str_ends_with($h, $n). (int|false → bool)',
            MutatorCategory::ORTHOGONAL_REPLACEMENT,
            null,
            <<<'DIFF'
- if (strpos($p, $suffix) !== false) { ... }

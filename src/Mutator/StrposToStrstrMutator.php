...
final class StrposToStrstrMutator implements Mutator
{
    ...
    public function mutate(Node $node): iterable
    {
        ...
        // strstr($haystack, $needle) → return string|false
        $args = [$node->args[0], $node->args[1]];
        yield new FuncCall(new Name('strstr'), $args, $node->getAttributes());
    }

    public static function getDefinition(): Definition
    {
        return new Definition(
            'Replaces strpos($h, $n) with strstr($h, $n). (int|false → string|false)',
